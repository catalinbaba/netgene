 ...
 * @author CBaba
 */
public class HingeError implements ErrorFunction
{
    private double maxError = 0.01;
    
    @Override
    public double calculateOutputError(DataSet outputSet, DataSet targetSet)
    {
        ...
        for(int i=0; i<targetSet.size(); i++)
        {
            ...
            {
                totalError = totalError + Math.max(0, 1.0 - target[j] * output[j]);
            }
        }
        System.out.println(totalError/targetSet.size());
