 ...
 * @author cbaba
 */
public class CustomCrossover2 extends CrossoverOperator
{
    ...
    public Offspring recombine(Individual x, Individual y) throws CrossoverException {
        ...
        for(int i=0; i<offspring.getSize(); i++)
        {
            ...
            for(int j=0; j<SudokuGame.sudoku.length; j++)
            {
                if(SudokuGame.sudoku[j] != 0)
                    indv.getChromosome().setGene(j, new IntegerGene(SudokuGame.sudoku[j])); 
            }
